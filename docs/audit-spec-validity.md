...
Отдельный случай — `lateinit`-свойства, которыми пользуются, чтобы дописать в команду то, чего в
запросе нет (локаль, principal). Рефлексия их пропускает: у такого свойства нет значения, пока
кто-то его не выставит, а выставляют его уже после чтения тела. Но эта догадка уместна только там,
где других сведений нет. `lateinit var` в `@Serializable`-классе — обычный элемент сериализатора:
...
// decodeFromString<Command>({"email":"a"}) →
// MissingFieldException: Field 'userId' is required
```

Ключ обязателен, `call.receive()` без него падает — значит, промолчать о таком свойстве означало бы
описать запрос, который сервер не принимает. Поэтому фильтр по `isLateinit` остался только на
рефлексивной ветке, а там, где есть сериализатор, решает он. Сказать «это поле заполняется позже»
можно ровно одним способом — `@Transient`, и его сериализатор сообщает сам.

Тесты: `SerializedShapeTest`, `LateInitPropertyTest`.
